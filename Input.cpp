...
void gInput::init()
{
	...
	if( !m_pDI )
	{
		MessageBoxA( 0, "Can't create DirectInput8!", "Error!", MB_OK|MB_ICONERROR|MB_SYSTEMMODAL );
		PostQuitMessage( -1 );
	}
	...
	if( !m_pKeyboard )
	{
		MessageBoxA( 0, "Can't create DirectInputDevice keyboard!", "Error!", MB_OK|MB_ICONERROR|MB_SYSTEMMODAL );
		PostQuitMessage( -1 );
	}
	...
	if( !m_pMouse )
	{
		MessageBoxA( 0, "Can't create DirectInputDevice mouse!", "Error!", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL );
		PostQuitMessage( -1 );
	}
	...
	if( FAILED( hr ) )
	{
		MessageBoxA( 0, "Can't can't set keyboard cooperative level!", "Error!", MB_OK|MB_ICONERROR|MB_SYSTEMMODAL );
		PostQuitMessage( -1 );
	}
	...
	if( FAILED( hr ) )
	{
		MessageBoxA( 0, "Can't can't set mouse cooperative level!!", "Error!", MB_OK|MB_ICONERROR|MB_SYSTEMMODAL );
		PostQuitMessage( -1 );
	}
