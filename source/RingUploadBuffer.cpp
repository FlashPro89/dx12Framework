...
RingUploadBuffer::RingUploadBuffer( ComPtr<ID3D12Device> cpDevice ) :
...
bool RingUploadBuffer::initialize( ADDRESS uploadBufferSize )
{
...
void* RingUploadBuffer::allocate( UINT frameId, ADDRESS size, ADDRESS align, ADDRESS* pOutOffset)
{
    void* lp = 0;
...
UINT64 RingUploadBuffer::getAvailableAllocationSize(ADDRESS align) const
{
    UINT64 _firstByte = reinterpret_cast<size_t>(m_lpMappedData);
    UINT64 _currentPos = m_uploadsQueue.empty() ? 0 : m_uploadsQueue.back().resourceEndOffset;
    UINT64 _alignedPos = _align(_currentPos + _firstByte, align) - _firstByte;
...
void RingUploadBuffer::frameEnded(UINT frameId)
{
...
void RingUploadBuffer::clearQueue()
{
    while( !m_uploadsQueue.empty() )
        m_uploadsQueue.pop();
}

ID3D12Resource* RingUploadBuffer::getResource() const
{
    return m_cpUploadBuffer.Get();
}

size_t RingUploadBuffer::getQueueSize() const
{
